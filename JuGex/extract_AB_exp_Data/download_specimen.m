...
function specimen = download_specimen(specimen_name,disp_url)
...
while isempty(t)
    str = urlread(request);
    t=strfind(str,'"success": true');
    if isempty(t)
        r = randi([1,5],1,1);
        d_str=['Failed to connect API Service. Waiting ' num2str(r) ' seconds... Another try...'];
        disp(d_str);
        pause(r);
        continue;
    end
